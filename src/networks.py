...
def qap_hierarchy(stats_path: str, n_perm: int = 199, max_windows: Optional[int] = 8,
                  min_nodes: int = 20, seed: int = 0) -> pd.DataFrame:
...
def build_all(data_dir: str, out_dir: str, stats_file: Optional[str] = None,
              ...
              mrqap_windows: Optional[int] = 8) -> None:
    set_style()
    ...
    if run_mrqap:
        ...
        if len(qap):
            log.info(f"QAP summary: mean r(E00,s_odd)={qap['r_e00_s_odd'].mean():.3f}, "
                     f"mean gate R^2(s_odd|pooled,e00)="
                     f"{qap['gate_r2_s_odd_on_pooled_e00'].mean():.3f}")
